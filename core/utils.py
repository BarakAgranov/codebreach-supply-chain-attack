"""
utils.py -- Shared utilities for the CodeBreach attack scripts.

Provides colored terminal output, structured JSON logging, retry logic,
table formatting, and build status polling used across all attack phases.
"""
import json
import os
import time
...
_PROGRESS_FILE = str(
    Path(__file__).resolve().parent.parent / "logs" / ".attack-progress.json"
)
...
def log_event(
    ...
    Never raises exceptions -- logging must not break the attack.
    """
...
def wait_for_build_completion(
    ...
    """
    Poll until a CodeBuild build completes.

    Args:
        client: boto3 CodeBuild client.
        build_id: The CodeBuild build ID to monitor.
        max_wait: Maximum seconds to wait (default: 5 minutes).
        poll_interval: Seconds between polls (default: 10).

    Returns:
        Final build status string, or None if timed out.
    """
    start = time.time()
    while time.time() - start < max_wait:
        try:
            ...
            if not builds:
                print_warning("Build not found, retrying...")
                time.sleep(poll_interval)
                continue

            status = builds[0].get("buildStatus", "UNKNOWN")
            phase = builds[0].get("currentPhase", "UNKNOWN")
            print_info(f"Build status: {status} (phase: {phase})")
    ...
    print_error(f"Timed out waiting for build after {max_wait}s")
    return None
...
def safe_api_call(
    ...
    **kwargs: Any,
    ...
    Retries on transient errors with exponential backoff. Immediately
    re-raises non-retryable errors (access denied, not found, etc.).
    """
